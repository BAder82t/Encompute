...
import encompute
from encompute import Tensor, EncomputeError, secret

W = [0.5, -0.25, 2.0]
encrypted = pytest.mark.skipif(not encompute.has_openfhe(), reason="built without OpenFHE")
...
def score(x: secret[Tensor[3], -1.0:1.0]):
    return encompute.sigmoid(encompute.dot(W, x) + 0.1)
...
def test_privacy_violations_have_codes():
    ...
    def select(x: secret[float, 0:1]):
        return encompute.select(x, 1.0, 0.0)
    ...
    for fn, code in cases.items():
        assert code_of(lambda: encompute.compile(fn)) == code, fn.__name__
...
def test_compiler_limits_have_codes():
    ...
    assert code_of(lambda: encompute.compile(deep)) == "ENC1201"
    assert code_of(lambda: encompute.compile(too_precise, precision=1e-15)) == "ENC1202"
    assert code_of(lambda: score([2.0, 0.0, 0.0])) == "ENC1102"
    assert code_of(lambda: score([0.0, 0.0])) == "ENC1102"
...
def test_numpy_publics_and_operators():
    ...
    def layer(x: secret[Tensor[3], -1:1], M=M, b=b):
        h = M @ x + b
        return {"h": h, "total": encompute.sum(h * h) / 2, "p": (x @ M.T) ** 3}

    m = encompute.compile(layer)
...
def test_public_parameters_bound_at_compile():
    def f(x: secret[Tensor[2], -1:1], w):
        return encompute.dot(w, x)

    m = encompute.compile(f, w=[2.0, 3.0])
    assert m([1.0, -1.0]) == pytest.approx(-1.0)
    assert code_of(lambda: encompute.compile(lambda x: x)) == "ENC1301"
...
def test_module_cli_compiles_python(tmp_path):
    ...
    out = tmp_path / "twice.encompute"
    r = subprocess.run([sys.executable, "-m", "encompute", "compile", str(src), "-o", str(out)],
