"""Tracing frontend: turns a typed Python function into Veil IR (.vlir text).
...
comparisons) raises a ``VeilError`` with a stable code instead.
"""
...
class VeilError(Exception):
    """A Veil diagnostic with a stable code such as ``VEIL1004``."""
...
def _err(code: str, message: str) -> VeilError:
    return VeilError(code, message)
...
def _num(x: float) -> str:
    ...
    if not math.isfinite(x):
        raise _err("VEIL1301", f"constant {x} is not finite")
    return repr(x)
...
class _Graph:
    ...
    def constant(self, value: Any) -> Tuple[int, str, Any]:
        ...
        if isinstance(value, bool):
            raise _err("VEIL1301", "booleans are not numbers in Veil")
        ...
        if isinstance(value, (list, tuple)) and value and all(
            ...
            if cols == 0 or any(len(r) != cols for r in value):
                raise _err("VEIL1301", "matrix rows must be non-empty and of equal length")
...
class Secret:
    """A secret value inside a ``@veil.compile`` function.
    ...
    def _operand(self, other: Any) -> Tuple[int, Optional[int], bool]:
        ...
        if isinstance(other, Secret):
            if other._g is not self._g:
                raise _err("VEIL1301", "secret values from different compilations cannot be mixed")
            return other._id, other._n, True
        i, kind, dims = self._g.constant(other)
        if kind == "matrix":
            raise _err("VEIL1301", "use `M @ x` or veil.matvec(M, x) for matrix products")
    ...
    def _elementwise(self, op: str, other: Any, reflected: bool = False) -> "Secret":
        ...
        if n is not None and on is not None and n != on:
            raise _err("VEIL1301", f"{op} needs equal lengths, got {n} and {on}")
    ...
    def __pow__(self, k: Any) -> "Secret":
        ...
        raise _err("VEIL1005", f"secret ** {k!r}: only positive integer powers are supported")
    ...
    def __bool__(self) -> bool:
        raise _err(
            ...
            "the value is encrypted at run time. Use arithmetic instead; veil.select arrives in 0.3",
        )
    ...
    def __getitem__(self, _: Any) -> Any:
        raise _err("VEIL1005", "indexing or slicing a secret vector is not supported in v0.1")

    def __iter__(self) -> Any:
        raise _err("VEIL1005", "iterating over a secret vector is not supported in v0.1")
...
def _secret_arg(name: str, x: Any) -> Secret:
    if not isinstance(x, Secret):
        raise _err("VEIL1301", f"veil.{name} needs a secret argument here")
    return x
...
def sum(x: Any) -> Any:  # noqa: A001 - mirrors numpy naming
    ...
    if isinstance(x, Secret):
        if x._n is None:
            raise _err("VEIL1301", "veil.sum needs a vector")
...
def dot(a: Any, b: Any) -> Any:
    ...
    if na is None or nb is None or na != nb:
        raise _err("VEIL1301", f"dot needs two vectors of equal length, got {na} and {nb}")
...
def matvec(m: Any, x: Any) -> Secret:
    ...
    if isinstance(m, Secret):
        raise _err("VEIL1005", "matvec needs a public matrix in v0.1")
    i, kind, dims = x._g.constant(m)
    if kind != "matrix":
        raise _err("VEIL1301", "matvec needs a 2-D public matrix")
    rows, cols = dims
    if x._n != cols:
        raise _err("VEIL1301", f"matvec: matrix has {cols} columns, vector has {x._n} elements")
...
def poly(x: Any, coeffs: Sequence[float]) -> Any:
    ...
    if isinstance(x, Secret):
        if len(coeffs) < 2:
            raise _err("VEIL1301", "poly needs at least two coefficients")
...
def select(*_: Any) -> Any:
    raise _err("VEIL1005", "veil.select (encrypted branching) needs TFHE comparisons, which arrive in 0.3")
...
def trace(fn: Any, precision: float, name: Optional[str], publics: Dict[str, Any]) -> Tuple[str, Outputs]:
    ...
    for pname, p in sig.parameters.items():
        ...
        if isinstance(ann, SecretSpec):
            ...
            raise _err(
                ...
                f"(veil.compile({fn.__name__}, {pname}=...))",
            )
    ...
    for oname, v in items:
        if not isinstance(v, Secret):
            raise _err(
                "VEIL1301",
                f"output {oname!r} does not depend on any secret input; compute it outside Veil",
            )
