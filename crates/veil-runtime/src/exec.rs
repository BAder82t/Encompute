...
use veil_ckks::{CkksPlan, Instr};
use veil_ir::{check_inputs, Inputs, Outputs, Program, Result};
...
pub fn evaluate_encrypted<B: CkksBackend>(
    ...
    for (i, instr) in plan.instrs.iter().enumerate() {
        let r = |reg: veil_ckks::Reg| {
            regs[reg.index()]
                .as_ref()
                .expect("register used after its last use")
        };
        let plain = |p: &veil_ckks::Plain| p.materialize(program, plan.slots);
        let ct = match instr {
