//! Compiled artifact: a directory `name.veil/` with
//! `program.vlir`, `plan.json`, `parameters.json`, `security.json` and
...
pub const FORMAT: &str = "veil-artifact/0.1";
const FILES: [&str; 4] = [
    "program.vlir",
    "plan.json",
    "parameters.json",
...
impl Model {
    ...
    pub fn save(&self, dir: &Path) -> Result<()> {
        ...
        let manifest = Manifest {
            format: FORMAT,
            veil_version: env!("CARGO_PKG_VERSION"),
    ...
    /// Load an artifact: verify every file hash, recompile `program.vlir`,
    ...
    pub fn load(dir: &Path) -> Result<Model> {
        ...
        let model = Model::from_vlir(&stored["program.vlir"])?;
        ...
        for name in ["plan.json", "parameters.json", "security.json"] {
            if fresh[name] != stored[name] {
                return Err(Error::new(
                    Code::Artifact,
                    format!(
                        "{name} differs from what this Veil version ({}) compiles; recompile the \
                         artifact (built with {})",
                        env!("CARGO_PKG_VERSION"),
                        manifest["veil_version"]
                    ),
                ));
            }
        }
