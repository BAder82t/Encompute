...
use serde::Serialize;
use veil_backend::{CkksBackend, MockBackend, MockConfig, MockSecretKey};
use veil_ckks::{compile, Compiled};
use veil_ir::{evaluate, parse, Code, Error, Inputs, Outputs, Program, Result};
...
enum Session {
    ...
    #[cfg(feature = "openfhe")]
    OpenFhe(veil_openfhe::OpenFheBackend, veil_openfhe::OpenFheSecretKey),
}
...
impl Model {
    ...
    pub fn from_vlir(text: &str) -> Result<Self> {
    ...
    fn new_session(&self, mode: Mode) -> Result<Session> {
        ...
        match mode {
            ...
            #[cfg(feature = "openfhe")]
            Mode::Encrypted => {
                let (b, sk) = veil_openfhe::OpenFheBackend::new(&c.params, &c.plan.rotations)?;
