...
use veil_runtime::{diff_test, run, sample_inputs};

fn exact_mock(c: &veil_ckks::Compiled) -> (MockBackend, veil_backend::MockSecretKey) {
    MockBackend::new(
        &c.params,
        &c.plan.rotations,
        MockConfig {
    ...
    fn lowering_preserves_semantics(p in arb_program()) {
        match compile(&p) {
            Ok(_) => {
                ...
                let scale = veil_analysis::ranges(&p).unwrap().max_abs().max(1.0);
