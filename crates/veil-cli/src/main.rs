...
use veil_ir::{Code, Error, Inputs, Result};
use veil_runtime::{Mode, Model};
...
#[derive(Subcommand)]
enum Cmd {
    /// Compile a `.vlir` file, or a Python function `file.py:function`, into a `.veil` artifact.
    Compile {
        source: String,
        /// Output directory (default: <name>.veil next to the source).
        #[arg(short, long)]
...
fn compile(source: &str, output: Option<PathBuf>) -> Result<ExitCode> {
    ...
    if path.extension().is_some_and(|e| e == "py") {
        ...
        cmd.args(["-m", "veil", "compile", source]);
        ...
        let status = cmd.status().map_err(|e| {
            Error::new(
                Code::Artifact,
                format!("could not run python3 -m veil: {e}"),
            )
        })?;
    ...
    let m = load(path)?;
    let out = output.unwrap_or_else(|| path.with_file_name(format!("{}.veil", m.program().name())));
    m.save(&out)?;
