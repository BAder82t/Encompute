//! Exact programs encrypted on OpenFHE (BinFHE): the production exact
//! backend, locally and remotely.
#![cfg(feature = "openfhe")]
...
/// The flagship on OpenFHE exact (BinFHE), locally and through an HTTP
/// evaluator that holds only the evaluation keys. `ENCOMPUTE_EXACT_CASES` sets
/// the differential-test size (default 20).

#[test]
fn openfhe_exact_end_to_end() {
    ...
    eprintln!(
        "openfhe-exact: {} cases, {} matches, {} mismatches",
    ...
    let run = remote
        ...
        .unwrap();
    assert_eq!(run.receipt.receipt.backend, "openfhe-exact");
    assert_eq!(run.receipt.receipt.scheme, "BinFHE");
    let (out, stats) = (run.outputs, run.stats);
    assert_eq!(out["approved"], vec![1.0]);

    // Another OpenFHE exact client: its inputs are refused (keys not registered),
    ...
    eprintln!(
        "remote openfhe-exact: evaluation keys {} MiB, request {} KiB, response {} KiB, evaluator {:.0} ms",
