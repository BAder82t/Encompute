...
use encompute_ckks::select_params;
use encompute_openfhe::{openfhe_choice, OpenFheBackend};
...
fn arithmetic_matches_plaintext() {
    ...
    let dec = |c: &encompute_openfhe::OpenFheCiphertext| be.decrypt(&sk, c).unwrap();
...
#[test]
fn encompute_parameter_choice_matches_openfhe() {
    for depth in [1, 2, 3, 5, 7, 9, 12, 16, 20] {
        for (max_abs, precision) in [(1.0, 1e-3), (30.0, 1e-4), (1.0, 1e-6)] {
            ...
            assert!(
                log_qp <= p.log_qp,
                "OpenFHE log QP {log_qp} > Encompute estimate {}",
                p.log_qp
            );
        }
    }
