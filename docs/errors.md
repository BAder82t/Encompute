...
| ENC2201 | coordinator, owner, fine-tuning preview | RELEASE DENIED: the release would exceed an asset's privacy budget. For DP-SGD fine-tuning, the preview denies a run whose planned rounds would exceed it, before training | Stop releasing from this asset, plan fewer rounds, or have its owners approve a new budget (a new policy) |
...
| ENC2301 | trust graph | A piece of evidence (receipt, record, spec) is malformed or does not verify | Re-export it from its source; do not add evidence that fails verification |
| ENC2302 | trust graph | An owner authorization or revocation is missing, signed by a key that is not the owner's, for an asset the party does not own, expired or revoked | Have the asset's owner approve the program with `encompute trust authorize` |
| ENC2303 | trust graph | The trust bundle is inconsistent: a dangling edge, a node that contradicts its evidence, broken lineage, or an unsupported version | Rebuild the bundle from its evidence; do not edit `trust.json` by hand |
| ENC2401 | planner | PLANNING FAILED: no combination of available mechanisms satisfies every trust requirement (the report lists every candidate and why it was refused) | Make a mechanism available (a TEE with a key broker, an FHE backend, execution proofs), relax the policy explicitly, or restructure the program |
| ENC2402 | planner, trust graph | A confidential execution plan is invalid: a requirement dropped or not discharged, a mechanism unavailable or without its prerequisites, or a plan for another program or policy | Regenerate the plan with `encompute plan`; never edit a plan by hand |
| ENC2403 | aggregation, trust graph | Execution does not match the approved plan: another PlanId, no plan, or a round or execution without a planned mechanism | Run under the approved plan (`--plan`), or approve a new plan |
| ENC2501 | training | A training worker, model, dataset, layout or checkpoint does not match the approved training spec (another model version, dataset, code, LoRA configuration, plan or policy) | Rebuild from the approved spec; a changed configuration needs a new spec and new approvals |
| ENC2502 | training | A checkpoint or sealed artifact is corrupted, from another project or run, or behind the authoritative privacy ledgers (stale: a later round was released) | Resume from the latest checkpoint; never restore an older one |
| ENC2503 | training | EXPORT DENIED: a derived adapter inherits a release restriction from its base model, a dataset or an update | Keep the adapter inside the collaboration, or have every parent's owner permit public adapters |
| ENC2504 | model import, worker | MODEL PACKAGE REFUSED: a mutable revision, remote code (`*.py`, `auto_map`, `trust_remote_code`), pickled weights, an unknown file, an unsupported architecture, or other library versions than the package binds | Import from an immutable commit with safetensors weights and a Transformers-native architecture; run the approved library versions |
| ENC2601 | control plane | Unauthenticated: no bearer token or service signature, an expired or forged token, a token from an untrusted issuer, or a development token in production | Log in again (`encompute login`); in production use the configured OIDC provider |
| ENC2602 | control plane | Forbidden: the identity is a member of the organization but lacks the role for this action | Ask an organization admin for the role |
| ENC2603 | control plane | Not found: no such resource visible to this identity. Resources owned by other organizations are reported the same way, so their existence is not revealed | Check the ID, and that a collaboration grants your organization access |
| ENC2604 | control plane | Conflict: an invalid job state transition, an idempotency key reused with a different request, a revoked asset, or a duplicate registration | Use a new idempotency key for a new request; revoked assets cannot be used |
| ENC2605 | control plane, services | Insecure configuration refused: production mode rejects development identities and tokens, development key stores, ephemeral databases, default credentials and secrets passed on the command line | Configure the production equivalents (OIDC, a root key provider, PostgreSQL, secret files) |
| ENC2606 | control plane | No registered, ready evaluator supports the job's backend and parameter profile | Register an evaluator with that capability, or wait for one to become ready |
| ENC2607 | services | A service request or message is unsigned, signed by an unknown or disabled service, addressed to another recipient, expired, or replayed | Check the service identities and clocks; replays are refused by design |
